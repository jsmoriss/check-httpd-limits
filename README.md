...
<ul>
<li><code>--save</code> : Save the current process average sizes to an SQLite database file (/var/tmp/check\_httpd\_limits.sqlite).</li>
...
<pre><code>$ sudo ./check\_httpd\_limits.pl --help
Syntax: ./check\_httpd\_limits.pl [--help] [--debug] [--verbose] [--exe=/path/to/httpd] [--swappct=#] [--save] [--days=#] [--maxavg]
...
--save         : Save process average sizes to database (/var/tmp/check\_httpd\_limits.sqlite).
